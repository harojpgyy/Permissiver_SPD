...
flash_dtbo() {
  ...
  if [ "$dtbo" ]; then
    dtboblock=/dev/block/platform/soc/soc:ap-ahb/20600000.sdio/by-name//tbo$slot;
    if [ ! -e "$dtboblock" ]; then
    ...
    boot|recovery)
      ...
      for name in $parttype; do
        for part in $name$slot $name; do
          ...
          elif [ -e /dev/block/platform/soc/soc:ap-ahb/20600000.sdio/by-name/$part ]; then
            target=/dev/block/platform/soc/soc:ap-ahb/20600000.sdio/by-name/$part;
          elif [ -e /dev/block/platform/soc/soc:ap-ahb/20600000.sdio/by-name/$part ]; then
            target=/dev/block/platform/soc/soc:ap-ahb/20600000.sdio/by-name/$part;
          elif [ -e /dev/block/platform/soc/soc:ap-ahb/20600000.sdio/by-name/$part ]; then
            target=/dev/block/platform/soc/soc:ap-ahb/20600000.sdio/by-name/$part;
